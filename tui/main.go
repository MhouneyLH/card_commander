...
func main() {
	...
	// create the boxes
	backlogBox := tview.NewTextView().SetBorder(true).SetTitle("Backlog")
	doBox := tview.NewTextView().SetBorder(true).SetTitle("Do")
	doneBox := tview.NewTextView().SetBorder(true).SetTitle("Done")
	reviewBox := tview.NewTextView().SetBorder(true).SetTitle("Review")
	...
	form.SetBorder(true).SetTitle("Manage Inventory").SetTitleAlign(tview.AlignCenter)

	first_flex := tview.NewFlex().
		AddItem(backlogBox, 0, 1, false).
		AddItem(doBox, 0, 1, false).
		AddItem(doneBox, 0, 1, false).
		AddItem(reviewBox, 0, 1, false)

	// Create a layout using Flex to display the inventory list and the form side by side
	flex := tview.NewFlex().SetDirection(tview.FlexRow).
		AddItem(first_flex, 0, 1, false).
